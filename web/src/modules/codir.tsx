...
function TrajectoryBar({ facture, certitudes, forecast, objectif }: { facture: number; certitudes: number; forecast: number; objectif: number }) {
  ...
  const seg = [
    ...
    { v: forecast, color: T.gold, label: "Pipeline pondéré" },
  ].filter((s) => s.v > 0);
  return (
    <div>
      ...
      <div className="mt-2.5 flex flex-wrap items-center gap-x-4 gap-y-1 text-[11px]">
        <Legend items={[{ color: T.emerald, label: "Facturé YTD" }, { color: T.steel, label: "Backlog projetable" }, { color: T.gold, label: "Pipeline pondéré" }]} />
        {gap > 0
...
export const Codir: FC<Props> = () => {
  ...
  const backlogCols = [
    ...
    colText("Affaire", (r: any) => <span className="truncate max-w-full sm:max-w-[380px] inline-block align-bottom">{r.affaire || "—"}</span>),
        ...
        {!att ? (
          ...
          <div className="flex flex-col gap-6 bg-panel p-5">
            ...
            <div className="grid gap-3 md:grid-cols-2 items-stretch">
              ...
              <GaugeCard title={`CAF prévisionnel vs objectif ${fy || ""}`} value={atteinte} num={cafEstYcForecast} objectif={objectifCaf} sub="projeté yc pipeline pondéré" />
            </div>
