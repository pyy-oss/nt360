...
/** Jauge d'atterrissage PARTAGÉE : arc coloré selon le taux d'atteinte (atteinteTone) + légende.
 *  Factorise la présentation répétée à l'identique (Vue d'ensemble · Prévision · Simulateur backlog). */
...
/** Série du funnel pondéré par étape (1→5) : Brut vs Pondéré. Partagée Pipeline ↔ CommercialCockpit
 *  (le tableau était construit à l'identique aux deux endroits). */
export function buildStageFunnel(byStage?: Record<number, { amount?: number; weighted?: number }>) {
  return [1, 2, 3, 4, 5].map((s) => ({ name: STAGE_SHORT[s], Brut: byStage?.[s]?.amount || 0, "Pondéré": byStage?.[s]?.weighted || 0 }));
}
