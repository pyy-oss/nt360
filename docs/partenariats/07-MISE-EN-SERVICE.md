...
Onglet **Paramétrage** → carte *Référentiel des partenaires* → bouton **« Nouveau partenaire »** (écriture `partenariats`). Le formulaire couvre, pour chaque constructeur (Dell, Cisco, Fortinet, Huawei…) : nom + programme, **niveaux** (`tiers`), **compétences** (`competencies`), **catalogue de certifications** (`certificationCatalog` avec `validityMonths` — ex. Fortinet 24 mois), et **exigences de quota** (`requirements` : par niveau, cible certif/compétence, minimum d'ingénieurs). Un partenaire existant se retouche via le bouton **Éditer** de sa ligne.

> Les identifiants techniques (niveau/compétence/certif) sont **dérivés des libellés** (slug), comme les codes de l'ERP. L'écriture **remplace** le référentiel du partenaire (sémantique « je pose l'état »). Intégrité vérifiée côté serveur : une exigence pointe un niveau + une cible connus. Le formulaire s'appuie sur le même callable `upsertParPartner` (accessible aussi en direct pour un import scripté).
