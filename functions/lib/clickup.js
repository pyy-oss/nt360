...
// par ClickUp sur un 429 ; sinon exponentiel (500 ms × 2^tentative) borné à 8 s. Le Retry-After serveur
// est honoré jusqu'à 60 s (une fenêtre de throttle ClickUp, par minute) — assez pour la purger sans
// stalls interminables ; un cap 30 s expirait le budget DANS la même fenêtre → 429 en cascade (C2).
...
async function api(token, method, path, body, opts) {
  ...
  const maxThrottleWaits = opts && Number.isFinite(opts.throttleWaits) ? opts.throttleWaits : 3;
  ...
  for (;;) {
    ...
    // 429 (throttling) : on OBÉIT au Retry-After serveur (ou back-off exponentiel s'il est absent) sur un
    // budget PROPRE `maxThrottleWaits`, et — point clé — le 429 n'entre JAMAIS dans le bloc 5xx ci-dessous.
    // Sinon les deux budgets s'ADDITIONNAIENT (un seul appel throttlé pouvait attendre 9×… → dépasser le
    // timeout de la fonction, cf. vérification C2). Le back-off 429 est indexé sur throttleWaits.
    if (res.status === 429 && throttleWaits < maxThrottleWaits) {
      await res.text().catch(() => {}); // draine le corps (évite une fuite de socket undici) avant retry
      await sleep(retryDelay(throttleWaits, res.headers && res.headers.get("retry-after")));
      throttleWaits++;
      continue;
    }
    if (res.status >= 500 && attempt < maxRetries) {
      await res.text().catch(() => {}); // draine le corps avant retry
