...
// === PLAN DE CHARGE / STAFFING (Lot 12 « 20/10 DirOps ») — affectations consultant × mission × période.
// assignments/* callable-only. Écriture « pipeline », lecture « overview ». staffingPlan calcule le plan
// de charge côté serveur (source unique domain/assignment) : charge par consultant/mois + sur/sous-charge.
exports.upsertAssignment = onCallG("upsertAssignment", { memoryMiB: 256, timeoutSeconds: 60 }, async (req) => {
  await requireWrite(req, "pipeline");
...
exports.deleteAssignment = onCallG("deleteAssignment", { memoryMiB: 256, timeoutSeconds: 60 }, async (req) => {
  await requireWrite(req, "pipeline");
...
exports.staffingPlan = onCallG("staffingPlan", { memoryMiB: 256, timeoutSeconds: 60 }, async (req) => {
  await requireRead(req, "overview");
...
exports.upsertCandidate = onCallG("upsertCandidate", { memoryMiB: 256, timeoutSeconds: 60 }, async (req) => {
  await requireWrite(req, "pipeline");
...
exports.deleteCandidate = onCallG("deleteCandidate", { memoryMiB: 256, timeoutSeconds: 60 }, async (req) => {
  await requireWrite(req, "pipeline");
...
exports.listCandidates = onCallG("listCandidates", { memoryMiB: 256, timeoutSeconds: 60 }, async (req) => {
  await requireRead(req, "overview");
...
// RENTABILITÉ PAR RESSOURCE (Lot 17 « 20/10 DirOps ») — P&L par consultant (CA réel = jours facturés ×
// TJM ; coût = jours ouvrés × CJM ; marge), agrégé global + par BU + par grade. DONNÉE CONFIDENTIELLE
// (coût/marge) → lecture gouvernée « rentabilite » (comme la marge P&L) — un commercial n'y accède pas.
exports.resourcePnl = onCallG("resourcePnl", { memoryMiB: 256, timeoutSeconds: 60 }, async (req) => {
