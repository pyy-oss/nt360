...
function validateTicket(d) {
  ...
  return { ok: true, value: { contratId, fp, client: cleanName(o.client) || "", titre, statut, priorite } };
}
...
function validateIntervention(d) {
  ...
  return { ok: true, value: { ticketId, contratId, fp, consultantId, date, heures: Math.round(heures * 100) / 100, commentaire: String(o.commentaire || "").trim().slice(0, 1000) } };
}
