...
function clickupBcSignals(bcLines, asOfMs) {
  ...
  const byNumber = new Map(); // N° BC CANONIQUE (bcKey) → agrégat (un BC = une tâche) : « BC-001 »/« BC 001 » fusionnés
  for (const b of bcLines || []) {
    const key = bcKey(b.bcNumber);
    if (!key) continue;
    const g = byNumber.get(key) || { bcNumber: String(b.bcNumber || "").trim(), supplier: b.supplier || "", linked: false, status: null, eta: null };
