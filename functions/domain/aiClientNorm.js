...
function buildClientNormPrompt(names) {
  ...
  const system =
    ...
    "clients. On te fournit la liste des graphies BRUTES rencontrées (avec leur fréquence). Identifie les " +
 ...
 */
function normalizeClientMergeSuggestions(parsed, names) {
  ...
  for (const m of (parsed && parsed.merges) || []) {
    ...
    const kf = canonicalKey(from), kt = canonicalKey(to);
