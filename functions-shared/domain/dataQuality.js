...
// Module PUR (testable).
const { fpKey, bcKey, num, plausibleYear, cleanName } = require("../lib/ids");
...
function issueDefs(orders, invoices, opps, bcLines, sheets, thr, staleOpps, agedOpps, rawOrders) {
  ...
  // N° BC CANONIQUE (bcKey) : « BC-001 » (Excel) et « BC 001 » (ClickUp) partagent la même clé → un doublon
  // à un séparateur/casse près ne passe plus entre les mailles (audit : double engagement fournisseur sinon).
  const bcDups = dupGroups(bcLines, (b) => [fpKey(b.fp), cleanName(b.supplier), b.amountXof, b.expenseType, bcKey(b.bcNumber)].map((v) => String(v ?? "")).join("|")).map((grp) => grp[0]);
